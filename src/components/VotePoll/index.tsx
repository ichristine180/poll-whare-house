...
export function VotePoll({ poll }: VotePollProps) {
  ...
  // Get tags from poll
  const tags = poll.tags?.map((t) => t.tag).filter(Boolean) || []
  ...
  const handleShare = (platform: string) => {
    ...
    const shareText = votedText ? `I voted ${votedText}. What about you?` : `${poll.question} - Vote now!`

    const baseUrl = typeof window !== 'undefined' ? window.location.origin : ''
    const pollPath = typeof window !== 'undefined' ? window.location.pathname : ''
    ...
    <div className="bg-white max-w-2xl mx-auto">
      ...
      {!hasVoted && heroImageUrl && (
        <div className="relative w-full h-48 sm:h-64 md:h-80 rounded-lg overflow-hidden mb-4">
          <Image src={heroImageUrl} alt={poll.question} fill className="object-cover" />
        </div>
      )}
      ...
      {!hasVoted && (
        <div className="space-y-3 mb-4">
          {poll.options?.map((option, index) => {
            ...
            return (
              <button key={index} onClick={() => setSelectedOption(index)} className={buttonClasses}>
      ...
      {!hasVoted && <p className="text-gray-600 text-base mb-4">{totalVotes.toLocaleString()} Votes</p>}
      ...
      {hasVoted && !showResults && results.length > 0 && (
        <div className="mb-6">
          ...
          {/* Share buttons */}
          {!poll.pollSettings?.hideShareOptions && (
            <div className="mt-4">
      ...
      {!hasVoted ? (
          ...
          {poll.description && (
            <div className="mb-6 bg-green-50 border border-green-200 rounded-lg p-4">
              <div className="flex items-center gap-2 mb-3">
                ...
                <h3 className="font-semibold text-green-700">About This Poll</h3>
              </div>
              <p className="text-gray-700 text-sm">{poll.description}</p>
            </div>
          )}

          {/* See Results / Take Another Poll */}
          <div className="space-y-3">
           
            <Link
              href="/"
              className="flex items-center justify-center gap-2 w-full py-3 border border-indigo-600 text-indigo-600 font-medium rounded-lg hover:bg-indigo-50 transition-colors"
            >
              Take Another Poll
            </Link>
          </div>
        ...
        <>
          ...
          <div className="mb-6">
                  ...
                  <div key={index} className="relative">
                    <div className="h-12 bg-gray-100 rounded-lg overflow-hidden">
                      ...
                      <div className="absolute inset-0 flex items-center justify-between px-4">
                        ...
                        <span className="font-bold text-gray-900">{option.percentage}%</span>
                      </div>
                    </div>
            ...
            <p className="text-center text-sm text-gray-500 mt-4">{totalVotes} total votes</p>
          </div>
          ...
          <Link
            href="/"
            className="flex items-center justify-center gap-2 w-full py-3 bg-indigo-600 text-white font-medium rounded-lg hover:bg-indigo-700 transition-colors"
          >
            Take Another Poll
          </Link>
        </>
